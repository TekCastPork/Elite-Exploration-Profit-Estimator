...
public class EventParser {
	...
	public static int determineEvent() {
		 ...
		 for(int i = 0; i < Resources.events.length; i++) {
			 if(event.equals(Resources.events[i])) {
				 Logger.printLog("We found a matching event.");
				 Logger.printLog("Event is:  " + Resources.events[i] + "  ("+i+")");
				 returnValue = i;
			 } else {
				 Logger.printLog("Event " + event + " does not match the indexed event " + Resources.events[i] + "  (" + i + ")");
			 }
		 }
		 return returnValue;
	}
	...
	public static String[] determineBodyType() {
		Logger.printLog("Determining Stellar Body's Type (Star/Planet");
		Logger.printLog("Trying a Stellar body first");
		...
		try {
   		 planetType[0] =  parser.getString("PlanetClass"); // run a function that figures out the planet/star's type	   		 
   	 	 ...
   	 	 planetType[1] = "Star";
   		 e.printStackTrace();
   		 Logger.printLog("PlanetClass doesnt exist....");    	
   		 Logger.printLog("Now trying for StarType");
   	 	}			
		try {
   		 ...
   		 e.printStackTrace();
   		 Logger.printLog("StarType doesnt exist....");    		 
   	 	}	
		return planetType;
	}
	/**
	...
	public static String[] gatherInfo(int type) {
		...
		switch(type) {
		case 0: // Continued
			Logger.printLog("Continued Event! The file has been closed and a new one is generated, we must re-run the file grabbing function!");
			Logger.printLog("Otherwise we won't get any new JSON to parse.");
			break;
		case 1:
			Logger.printLog("We bought ammo, lets get the price and modify some variables later on.");
		case 2:
			Logger.printLog("We bought drones, lets get the price and modify some variables later on.");
		case 3:
			Logger.printLog("We bought Exploration Data, lets get the price and modify some variables later on.");
		case 4:
			Logger.printLog("We bought Trade Data, lets get the price and modify some variables later on.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("Cost"));
			break;
		case 5:
			Logger.printLog("We jumped somewhere!");
			output[0] = "Jump";
			output[1] = parser.getString("StarSystem");
			output[2] = Double.toString(parser.getDouble("FuelLevel"));
			break;
		case 6:
			Logger.printLog("Vegeta, what does the scanner say about his boost level?");
			Logger.printLog("It's OVER 4!");
			output[0] = "Boost";
		...
		case 8:
			...
			break;
		case 9:
			Logger.printLog("We sold something to the market.");
			output[0] = "CashGain";
			output[1] = Integer.toString(parser.getInt("TotalSale"));
			break;
		case 10:
			Logger.printLog("We completed a mission.");
			output[0] = "CashGain";
			output[1] = Integer.toString(parser.getInt("Reward"));
			break;
		case 11:
			Logger.printLog("We bought a module for our ship.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("BuyPrice"));
			break;
		case 12:
			Logger.printLog("We retrieved a module.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("Cost"));
			break;
		case 13:
			Logger.printLog("We sold a module.");
			output[0] = "CashGain";
			output[1] = Integer.toString(parser.getInt("SellPrice"));
			break;
		case 14:
			Logger.printLog("We remotely sold a module.");
			output[0] = "CashGain";
			output[1] = Integer.toString(parser.getInt("SellPrice"));
			break;
		case 15:
			Logger.printLog("We payed some fines.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("Amount"));
			break;
		case 16:
			Logger.printLog("We payed some legacy fines.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("Amount"));
			break;
		case 17:
			Logger.printLog("We got our salary from Powerplay.");
			output[0] = "CashGain";
			output[1] = Integer.toString(parser.getInt("Amount"));
			break;
		case 18:
			Logger.printLog("We completely refuelled.");
			output[0] = "Fuel";
			output[1] = Double.toString(parser.getDouble("Amount"));
			output[2] = Integer.toString(parser.getInt("Cost"));
			break;
		case 19:
			Logger.printLog("We partially refuelled.");
			output[0] = "Fuel";
			output[1] = Double.toString(parser.getDouble("Amount"));
			output[2] = Integer.toString(parser.getInt("Cost"));
			break;
		case 20:
			Logger.printLog("We repaired something.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("Cost"));
			break;
		case 21:
			Logger.printLog("We repaired everything.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("Cost"));
			break;
		case 22:
			Logger.printLog("We restocked our vehicles.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("Cost"));
			break;
		case 23:
			Logger.printLog("We are respawning because we died.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("Cost"));
			break;
		case 24:
			Logger.printLog("We scanned something!");
			Logger.printLog("we need to gather alot of data about the scanned body.");
			...
			break;
		case 25:
			Logger.printLog("We sold some drones.");
			output[0] = "CashGain";
			output[1] = Integer.toString(parser.getInt("TotalSale"));
			break;
		case 26:
			Logger.printLog("We sold our exploration data.");
			output[0] = "ExplorationSell";
			output[1] = Integer.toString(parser.getInt("BaseValue")+ parser.getInt("Bonus"));
			break;
		case 27:
			Logger.printLog("We bought something from the shipyard.");
			output[0] = "CashLoss";
			output[1] = Integer.toString(parser.getInt("ShipPrice"));
			break;
		case 28:
			Logger.printLog("We sold something to the shipyard.");
			output[0] = "CashGain";
			output[1] = Integer.toString(parser.getInt("ShipPrice"));
			break;
		case 29:
			Logger.printLog("We scooped fuel.");
			output[0] = "FuelScoop";
			output[1] = Double.toString(parser.getDouble("Total"));
			break;
		case 30:
			Logger.printLog("Grabbing the location.");
			output[0] = "Location";
			output[1] = parser.getString("StarSystem");
		default:
			Logger.printLog("The Event was not in the list that we care about.");
			output[0] = "Unknown";
	...
	public static int getBodyValue(String classification, String terraformState, String type) {
		...
		if(type.equals("Star")) { // The type of body is a Star according to some previous functions that determined the class
			...
			for(int i = 0; i < Resources.bodyClasses.length; i++) {
				if(classification.equals(Resources.bodyClasses[i])) {
					Logger.printLog("We found the type in the bodyClasses area.");
					Logger.printLog("Now we must determine the value depending on the terraform state.");	
					Logger.printLog("The terraform state is: " + terraformState);
					if(terraformState.equals("null") || terraformState.equals("")) {
						Logger.printLog("It is not terraformable.");
						outputValue = bodyValueNonTerraform[i];												
					} else {						
						Logger.printLog("It's terraformable.");
